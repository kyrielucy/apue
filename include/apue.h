...
int     fd_pipe(int *);
int     recv_fd(int, ssize_t (*func)(int, const char *, size_t));
int     send_fd(int, int);
...
int     lock_reg(int, int, int, off_t, int, int, off_t);
...
#define wirte_lock(fd, offset, whence, len) \
...
#define is_read_lockable(fd, offset, whence, len) \
  (lock_teset((fd), F_RDLCK, (offset), (whence), (len)) == 0)

#define is_write_lockable(fd, offset, whence, len) \
  (lock_teset((fd), F_WRLCK, (offset), (whence), (len)) == 0)
